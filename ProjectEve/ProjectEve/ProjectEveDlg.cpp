...
void CProjectEveDlg::OnBnClickedButtonProcess()
{
	...
	UpdateData(true);
	RGBImage* image;
	image = new RGBImage[numberOfImage];//numberOfImage
	...
	snprintf(args, sizeof(args), "%d:%d", img_width, img_height);
	insert_filter(&vfilter_ctx, "scale", args, 0);
	set_video_filter(&vfilter_ctx, vfilter_ctx.inputs, vfilter_ctx.outputs);

	for (int i = 0; i < numberOfImage; i++) {
		snprintf(args, sizeof(args), "circle%d.jpg", i+1);
		if (OpenImage(args)) {
			image[i].copyFrame(pictureFrame);
			//av_free(pictureFrame);
		}
	}
	...
	for (int i = 0; i < m_filter_list.GetCount(); i++) {
		m_filter_list.GetText(i, temp);
		if (temp == "Scale filter") {
			snprintf(args, sizeof(args), "%d:%d", m_scale_width, m_scale_height);
			insert_filter(&vfilter_ctx, "scale", args, 0);
		}
		else if (temp == "Vflip filter")
			insert_filter(&vfilter_ctx, "vflip", NULL, 0);
		else if (temp == "Hflip filter")
			insert_filter(&vfilter_ctx, "hflip", NULL, 0);
		else if (temp == "Negative filter")
			insert_filter(&vfilter_ctx, "negate", NULL, 0);
		else  if (temp == "Edge filter")
			insert_filter(&vfilter_ctx, "convolution", "0 1 0 1 -4 1 0 1 0:0 1 0 1 -4 1 0 1 0:0 1 0 1 -4 1 0 1 0:0 1 0 1 -4 1 0 1 0:5:5:5:1:0:128:128:128", 0);
	}
	...
	for (int i = 0; i < 6; i++) {
		CPoint to(0, 0);
		CPoint from((i+1)*50, 220);

		test[i].SetToPos(to);
		test[i].SetFromPos(from);
		test[i].SetImage(i);
	}
	...
	m_status_text.SetWindowText(text);
	int i = 0;
	int frames = ifmt_ctx->streams[0]->nb_frames;
	m_progress.SetRange(0, frames);
	m_progress.SetPos(i);
	...
	while (av_read_frame(ifmt_ctx, &read_pkt) >= 0) {
		...
		if (read_pkt.stream_index == nVSI) {
			...
			if (avcodec_decode_video2(dec_V_ctx, pVFrame, &bGotPicture, &read_pkt) >= 0) {
				...
				//	int W = m_picture_frame->linesize[0];
					int W = filtered_frame->linesize[0];
					int i_W = pictureFrame->linesize[0];
					bool bCircle = FALSE;
					
					for (int k = 0; k < 6; k++) {
						int d_y = test[k].GetCurrentPosition().y;
						int d_x = test[k].GetCurrentPosition().x;
						test[k].goup();
						for (int i = 0; i < i_H; i++) {
							for (int j = 0; j <img_width; j++) {
		
								if (!image[test[k].GetImageIndex()].checkPixelWhite(i*i_W + j))
								{
									if (!bCircle)
									{
										bCircle = TRUE;
									...
									image[test[k].GetImageIndex()].getPixelColor(s_R, s_G, s_B, i*i_W + j);
...
void CProjectEveDlg::OnBnClickedButtonDebug()
{
	...
	for (int i = 0; i < (dst_height); i++) {
		for (int j = 0; j < dst_width; j++) {
			//m_picture_frame->data[0][i*dst_width + j] = (m_picture_frame->data[0][i*dst_width + j] / 32) * 32;//0~255 Y
			if (i < 300) {
				m_picture_frame->data[1][((i*dst_width) + j) / 2] = 255;//0~255 u

			}
			//m_picture_frame->data[2][(i*dst_width + j) / 4] = 60;//0~255 v
...
void CProjectEveDlg::OnBnClickedButtonNext()
{
	int bGotPicture = 1;
	int bGotSound = 1;
	if(av_read_frame(ifmt_ctx, &read_pkt) >= 0) {
		///> Decoding
